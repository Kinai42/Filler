...
/*   Updated: 2018/08/09 03:23:01 by dbauduin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			if (p->piece[py][px] == '*')
			{
				...
				while (++o < 4)
					if ((px + x + o) < map->size_x && (px + x - o) > 0 &&
						...
						if (ft_strrchr(map->adv, map->map[py + y][px + x + o]) ||
							ft_strrchr(map->adv, map->map[py + y][px + x - o]) ||
							ft_strrchr(map->adv, map->map[py + y + o][px + x]) ||
							ft_strrchr(map->adv, map->map[py + y - o][px + x]))
							ret += (4 - o);
			}
	}
